...
(function($) {
    $.fn.makeAsyncUploader = function(options) {
        return this.each(function() {
            ...
            //$('#upload_pdf_file_filename').val(id);
            ...
            var defaults = {
                ...
                upload_success_handler: function(file, response) {
                    $("#upload_pdf_file_filename").val(file.name);
                    $("input[name$=_guid]", container).val(response);
                    $("span[id$=_completedMessage]", container).html("Uploaded <b>{0}</b> ({1} bytes)"
                                .replace("{0}", file.name).replace("{1}", Math.round(file.size))
                            );
                    //$("span[id$=_completedMessage]", container).append( response );
                },
